...
test('renders an error state', async()=>{
 vi.mocked(api.getAssistantQualityDashboard).mockRejectedValue(new Error('failed'));
 render(<AssistantQualityPage/>);
 expect(await screen.findByText('Unable to load AskLeaveMaestro quality')).toBeInTheDocument();
});

test('does not render a blank screen for an incomplete dashboard response', async()=>{
 vi.mocked(api.getAssistantQualityDashboard).mockResolvedValue({requests:0} as Awaited<ReturnType<typeof api.getAssistantQualityDashboard>>);
 render(<AssistantQualityPage/>);
 expect(await screen.findByText('AskLeaveMaestro Quality')).toBeInTheDocument();
 expect(screen.getByText('No assistant requests in this period')).toBeInTheDocument();
});
