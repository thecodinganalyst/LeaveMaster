...
class TenantControllerSecurityTest {
    ...
    @Test
    @WithMockUser(username = "PlatformAdmin", authorities = {RbacPermissions.TENANT_READ, RbacPermissions.TENANT_WRITE})
    void platformAdminAuthoritiesAllowTenantCrud() throws Exception {
        ...
        mockMvc.perform(delete("/tenants/T1").with(csrf()))
    ...
    @Test
    @WithMockUser(username = "tenant-reader", authorities = RbacPermissions.TENANT_READ)
    ...
    @Test
    @WithMockUser(username = "ordinary-user", authorities = RbacPermissions.STAFF_READ)
