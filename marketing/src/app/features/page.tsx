    ...
    <section className="section">
      <div className="grid gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
        <div>
          ...
          <p className="mt-5 max-w-xl text-lg">A connected leave workflow for HR, employees, and approvers.</p>
        </div>
        <LeaveWorkflowVisual />
      </div>
        ...
        {features.map(([kind, title, description, bullets]) => (
          <article key={title} className="card">
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              {bullets.map((bullet) => (
                <div key={bullet} className="rounded-xl bg-brand-50 px-4 py-3 text-sm font-medium text-slate-700">✓ {bullet}</div>
              ))}
            </div>
          </article>
        ))}
      ...
      <div className="mt-12 grid gap-5 rounded-[2rem] bg-slate-50 p-8 sm:grid-cols-3 sm:p-10">
        <div>
          <FeatureIcon kind="approval" />
          <h3 className="mt-4 text-lg font-semibold">Controlled access</h3>
          <p className="mt-2 text-sm">Roles and permissions keep actions scoped to the right users.</p>
        </div>
        <div>
          <FeatureIcon kind="selfService" />
          <h3 className="mt-4 text-lg font-semibold">Flexible sign-in</h3>
          <p className="mt-2 text-sm">Username/password plus supported OAuth/OIDC options for provisioned users.</p>
        </div>
        <div>
          <FeatureIcon kind="assistant" />
          <h3 className="mt-4 text-lg font-semibold">Ask LeaveMaestro</h3>
          <p className="mt-2 text-sm">Help authorised users understand LeaveMaestro data and policy outcomes.</p>
        </div>
      </div>
      ...
      <div className="mt-12 rounded-[2rem] border border-brand-100 bg-brand-50 px-8 py-10">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="text-3xl font-bold text-slate-950">Explore the workflow yourself</h2>
            <p className="mt-3 text-slate-600">Use the demo or talk to us about your policies and jurisdictions.</p>
          </div>
