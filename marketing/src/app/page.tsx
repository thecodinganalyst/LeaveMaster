...
export const metadata: Metadata = {
  ...
  description: 'LeaveMaestro helps growing companies replace spreadsheet-based leave administration with jurisdiction-aware leave policies, entitlements, eligibility, proration, calendars, self-service, and approvals.',
};
...
export default function HomePage() {
  return (
    <>
      <section className="bg-white">
        <div className="section pt-16 sm:pt-20">
          <div className="grid gap-12 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
            <div>
              <p className="mb-4 inline-flex rounded-full border border-brand-100 bg-brand-50 px-4 py-2 text-sm font-medium text-brand-700">Leave management without the spreadsheet</p>
              ...
              <p className="mt-6 max-w-xl text-lg text-slate-600">Policies, entitlements, requests, approvals, and balances in one jurisdiction-aware workflow.</p>
              <div className="mt-8 flex flex-col gap-4 sm:flex-row"><CTAButton href="/demo">Try Demo</CTAButton><CTAButton href="/features" variant="secondary">Explore Features</CTAButton></div>
            </div>
            <HeroProductVisual />
          </div>
        </div>
      ...
      <section className="section pt-4">
        <div className="mb-10 max-w-2xl"><p className="text-sm font-semibold uppercase tracking-[0.2em] text-brand-600">Inside LeaveMaestro</p><h2 className="mt-3 text-3xl font-bold tracking-tight">See the workflow before you try it</h2><p className="mt-3 text-slate-600">Representative product snapshots show how policy configuration becomes everyday employee and manager actions.</p></div>
      ...
      <section className="section pt-4">
        <div className="rounded-[2rem] border border-brand-100 bg-brand-50 px-8 py-10 sm:px-10"><div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between"><div><h2 className="text-3xl font-bold text-slate-950">See LeaveMaestro in action</h2><p className="mt-3 max-w-2xl text-slate-600">Explore the demo or discuss your leave policies and rollout needs.</p></div><div className="flex flex-col gap-3 sm:flex-row"><CTAButton href="/demo">Try Demo</CTAButton><CTAButton href="/contact" variant="secondary">Contact Us</CTAButton></div></div></div>
      </section>
    </>
  );
}
